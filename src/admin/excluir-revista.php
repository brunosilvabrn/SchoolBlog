...
if(!isset($url[2])) {
	...
	echo "<script>window.location.href='administrador/revista'</script>";
		...
		<main>
			<div class="msg-excluir">
				...
				<div class="btn-excluir">
					<a class="btn btn-outline-success" href="administrador/excluir-revista/'.$valor['id'].'/deletar" role="button">SIM</a>
			...
			if(isset($url[3]) && $url[3] == 'deletar' && isset($url[2])) {
				...
				$sql = $pdo->prepare("DELETE FROM revista WHERE id = :id");
				$sql->bindValue(":id", $id);
				$sql->execute();
